...
class VisualizeDataset(PipelineWorker):
    def __init__(self, nb_images: int) -> None:
        super().__init__('visualize dataset', 'visualize')
        self.nb_images = nb_images

    def apply(self, target_ws: Workspace):
        df = Dataframes.from_csv(self.ctx.project_ws.path_to('dataset/train.csv')).head(self.nb_images)
        df = shuffle(df)

        images = df['id'] \
            .apply(lambda x: '{}/train/{}.tif'.format(self.ctx.root_ws.path, x)) \
