...
class ValidateTraining(Worker):
    ...
    def run(self, ws: Workspace):
        ...
        history_viewer.write(ws.path_to('training.png'))

        # confusion matrix -------------------------------------
        dataset = TrainDataset.from_path(ws.root.get_ws(self.dataset_path)).test

        result = self.make_predictions(dataset, ws)
        ...
        cm.save(ws.path_to('confusion_matrix'), normalize=True)
        cm.show()

        errors = result[dataset.df[dataset.y_col] != result[dataset.y_col]]
        errors.to_csv(ws.path_to('errors.csv'), index=False)
        y_classes = list(dataset.df[dataset.y_col].unique())

        for category in y_classes:
            errors_path = ws.path_to('errors_{}.csv'.format(category))
        ...
        #     doubtful.to_csv(ws.path_to('doubtful_{}.csv'.format(doubt)), index=False)
    ...
    def make_predictions(self, dataset: FileDataset, ws: Workspace):
        ...
        predictions.to_csv(ws.path_to('test_predictions.csv'), index=False)
        return predictions
